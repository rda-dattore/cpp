...
public:
  ...
  struct SurfaceReport {
    ...
    struct Data {
	Data() : slp(0.), stnp(0.), tdry(0.), tdew(0.), twet(0.), rh(0.),
            dd(0.), ff(0.) { }

	float slp, stnp, tdry, tdew, twet, rh, dd, ff;
    } data;
    struct Flags {
	Flags() : slp(' '), stnp(' '), tdry(' '), tdew(' '), twet(' '), rh(' '),
            dd(' '), ff(' ') { }
  ...
  struct SurfaceAdditionalData {
    ...
    struct Data {
	Data() : tmax(0.), tmin(0.), pcp_amt(0.), snowd(0.), vis(0.),
            tot_cloud(0.), cloud_base_hgt(0.), pres_tend(0.) { }

	float tmax, tmin, pcp_amt, snowd, vis, tot_cloud, cloud_base_hgt,
            pres_tend;
    } data;
    struct Flags {
...
public:
  ...
  ADPSurfaceReport() : synoptic_date_time_(), receipt_date_time_(),
      report_type_(), adp_addl_data() { }
...
public:
  ...
  char ceiling_determination_code() const
      { return ceiling_determination_code_; }
  void fill(const unsigned char *stream_buffer, bool fill_header_only);
  void print(std::ostream& outs) const;
  void print_header(std::ostream& outs, bool verbose) const;
...
private:
  ...
  friend void decode_ASOS_AWOS_present_weather(char *buf,
      std::vector<std::string>& vals);
  friend void decode_atmospheric_pressure_change(char *buf,
      std::vector<std::string>& vals);
  friend void decode_automated_present_weather(char *buf,
      std::vector<std::string>& vals);
  friend void decode_extreme_air_temperature(char *buf,
      std::vector<std::string>& vals);
  friend void decode_ground_surface_observation(char *buf,
      std::vector<std::string>& vals);
  friend void decode_ground_surface_minimum_temperature(char *buf,
      std::vector<std::string>& vals);
  friend void decode_hourly_liquid_precipitation(char *buf,
      std::vector<std::string>& vals);
  ...
  friend void decode_snow_accumulation(char *buf, std::vector<std::string>&
      vals);
...
public:
  struct WMSSCAdditionalData {
    WMSSCAdditionalData() : t_dep(0.), mois(0.), mois_dep(0.), sst(0.),
        sst_dep(0.), mois_flag(' '), n_pcp_1mm(0), pcp_dep(0), quint_pcp(0),
        nobs(0), hrs_sun(0), pct_avg_sun(0) { }

    float t_dep, mois, mois_dep, sst, sst_dep;
    unsigned char mois_flag;
    int n_pcp_1mm, pcp_dep, quint_pcp, nobs, hrs_sun, pct_avg_sun;
  };
  WMSSCObservation() : wmssc(), monthly_means(), annual_slp_indicator2(),
      annual_wmssc_addl_data(), annual_z(0) { }
  void fill(const unsigned char *stream_buffer, bool fill_header_only);
  ...
  bool has_additional_data() const
      { return (static_cast<int>(wmssc.addl_data_indicator) == 1); }
...
private:
  struct WMSSC {
    WMSSC() : data_source(), id_source(), ship_indicator(),
        addl_data_indicator(), format(0), name() { }
  ...
  struct MonthlyMeans {
    MonthlyMeans() : report(), addl_data(), slp_indicator2(), z(0),
        wmssc_addl_data() { }
...
public:
  ...
  struct CPCMonthlySummaryData {
    float tmean, tmax, tmin, hmax, rminl, ctmean, aptmax, wcmin, hmaxapt,
        wclminl, rpcp, epcp, cpcp, epcpmax, ahs, avp, apet, avpd, trad;
  ...
  CPCMonthlySummaryData CPC_monthly_summary_data() const
      { return cpc_mly_summ; }
  void fill(const unsigned char *stream_buffer, bool fill_header_only);
  void print(std::ostream& outs) const;
  void print_header(std::ostream& outs, bool verbose) const;
