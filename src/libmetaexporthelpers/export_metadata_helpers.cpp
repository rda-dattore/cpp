...
void fill_geographic_extent_data(Server& server, string dsid, XMLDocument&
    dataset_overview, double& min_west_lon, double& min_south_lat, double&
    max_east_lon, double& max_north_lat, bool& is_grid, vector<
    HorizontalResolutionEntry> *hres_list, my::map<Entry>
    *unique_places_table) {
  min_west_lon = min_south_lat = 9999.;
  max_east_lon = max_north_lat = -9999.;
  is_grid = false;
  auto ds_set = to_sql_tuple_string(ds_aliases(ng_gdex_id(dsid)));
  LocalQuery query("select definition, def_params from (select distinct "
      "grid_definition_code from \"WGrML\".summary where dsid in " + ds_set +
      ") as s left join \"WGrML\".grid_definitions as d on d.code = s."
  ...
  if (query.num_rows() > 0) {
    ...
    for (const auto& row : query) {
      ...
      if (gridutils::filled_spatial_domain_from_grid_definition(row[0] + "<!>" +
        ...
        if (hres_list != nullptr && unique_hres_set.find(hre.key) ==
            unique_hres_set.end()) {
    ...
    if (!elist.empty()) {
      ...
      for (const auto& element : elist) {
        ...
        if (gdef == "latLon" || gdef == "gaussLatLon") {
          def_params = element.attribute_value("numX") + ":" + element.
              attribute_value("numY") + ":" + element.attribute_value(
              "startLat") + ":" + element.attribute_value("startLon") + ":" +
              element.attribute_value("endLat") + ":" + element.attribute_value(
              "endLon") + ":" + element.attribute_value("xRes") + ":";
          if (gdef == "latLon") {
          ...
          def_params = element.attribute_value("numX") + ":" + element.
              attribute_value("numY") + ":" + element.attribute_value(
              "startLat") + ":" + element.attribute_value("startLon") + ":60" +
              element.attribute_value("pole") + ":" + element.attribute_value(
              "projLon") + ":" + element.attribute_value("pole") + ":" +
              element.attribute_value("xRes") + ":" + element.attribute_value(
              "yRes");
          ...
          def_params = element.attribute_value("numX") + ":" + element.
              ...
              "startLat") + ":" + element.attribute_value("startLon") + ":" +
              element.attribute_value("endLat") + ":" + element.attribute_value(
              "endLon") + ":" + element.attribute_value("xRes") + ":" + element.
              attribute_value("yRes") + ":" + element.attribute_value("resLat");
        } else if (gdef == "lambertConformal") {
          def_params = element.attribute_value("numX") + ":" + element.
              attribute_value("numY") + ":" + element.attribute_value(
              "startLat") + ":" + element.attribute_value("startLon") + ":" +
              element.attribute_value("resLat") + ":" + element.attribute_value(
              "projLon") + ":" + element.attribute_value("pole") + ":" +
              element.attribute_value("xRes") + ":" + element.attribute_value(
              "yRes") + ":" + element.attribute_value("stdParallel1") + ":" +
              element.attribute_value("stdParallel2");
        }
  ...
  if (hres_list != nullptr) {
    ...
    [](const HorizontalResolutionEntry& left, const HorizontalResolutionEntry&
        right) -> bool {
  ...
  if (table_exists(server, "WObML." + dsid + "_geobounds")) {
    query.set("select min(min_lat), min(min_lon), max(max_lat), max(max_lon) "
        "from \"WObML\"." + dsid + "_geobounds where min_lat >= -900000 and "
        "min_lon >= -1800000 and max_lat <= 900000 and max_lon <= 1800000");
    if (query.submit(server) == 0) {
      for (const auto& row : query) {
        if (row[0].empty() || row[1].empty() || row[2].empty() || row[3].
            empty()) {
          cerr << "oai warning: " << dsid << " geobounds: '" << row[0] << "','"
              << row[1] << "','" << row[2] << "','" << row[3] << "'" << endl;
        } else {
    ...
    query.set("select l.keyword, d.box1d_row, d.box1d_bitmap_min, d."
        "box1d_bitmap_max from search.locations as l left join search."
        "location_data as d on d.keyword = l.keyword and d.vocabulary = l."
        "vocabulary where l.dsid in " + ds_set + " and d.box1d_row >= 0");
    query.submit(server);
    if (query.num_rows() > 0) {
      for (const auto& row : query) {
        Entry entry;
        if (unique_places_table != nullptr && !unique_places_table->found(row[
            0],entry)) {
          entry.key = row[0];
      ...
      query.set("select keyword, min(box1d_row), min(box1d_bitmap_min), max("
          "box1d_row), max(box1d_bitmap_max) from search.location_data where " +
          where_conditions + " and box1d_row >= 0 group by keyword");
      query.submit(server);
      if (query.num_rows() > 0) {
        for (const auto& row : query) {
          if (row[2] != "0" || row[4] != "359") {
            convert_box_data(row[1], row[2], min_south_lat, min_west_lon,
                "min");
            convert_box_data(row[3], row[4], max_north_lat, max_east_lon,
                "max");
          }
        }
      }
