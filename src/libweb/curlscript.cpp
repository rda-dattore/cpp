...
void create_curl_script(std::list<std::string>& filelist,std::string directory,std::string dsnum,std::string script_type,std::string parameters,std::string level,std::string product,std::string start_date,std::string end_date)
{
  ...
  for (auto& file : filelist) {
    ...
    std::string inv_file="/usr/local/www/server_root/web/datasets/ds"+dsnum+"/metadata/inv/"+file+".GrML_inv";
    if (stat(inv_file.c_str(),&buf) != 0) {
	std::stringstream iss;
	system(("/usr/local/www/server_root/web/cgi-bin/datasets/inventory "+dsnum+" GrML "+file+" 2>&1 1>/dev/null").c_str());
    }
