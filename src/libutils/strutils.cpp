...
void replace_all(string& s, const string& old_s, const string& new_s) {
  ...
  size_t pos = 0;
  auto index = s.find(old_s, pos);
  while (pos < s.length() && index != string::npos) {
    s.replace(index, old_s.length(), new_s);
    pos = index + new_s.length();
    index = s.find(old_s, pos);
  }
}

void strput(char *string, int numeric, int num_chars, char fill, bool
    is_signed) {
  int off = 0;
  auto sign = ' ';
  if (numeric < 0) {
  ...
  if (numeric >= pow(10., num_chars)) {
    std::cerr << "Error in strput: integer " << numeric << " is larger than "
        << "character length " << num_chars << std::endl;
    exit(1);
  }
  auto lead = true;
...
void trim_front(string& s) {
  ...
  size_t n = 0;
  while (n < s.length() && (s[n] == ' ' || s[n] == 0x9 || s[n] == 0xa || s[n] ==
      0xd)) {
    ++n;
  }
  if (n > 0) {
...
bool is_alphanumeric(const string& s) {
  for (size_t n = 0; n < s.length(); ++n) {
    if ((s[n] < '0' || s[n] > '9') && (s[n] < 'A' || s[n] > 'Z') && (s[n] < 'a'
        || s[n] > 'z')) {
      return false;
    }
  }
...
size_t occurs(const string& s, const string& find_s) {
  size_t num_occurs = 0;
  size_t pos = 0;
  auto index = s.find(find_s, pos);
  while (pos < s.length() && index != string::npos) {
    ++num_occurs;
    pos = index + find_s.length();
    index = s.find(find_s, pos);
  }
  return num_occurs;
}
...
std::vector<std::pair<string, size_t>> split_index(const string& s, const
    string& separator) {
  ...
  int check_len = s.length() - separator.length() + 1;
  if (check_len < 1) {
    parts.emplace_back(make_pair(s, 0));
    return parts;
  }
  size_t start = 0;
  size_t end = 0;
  ...
  while (static_cast<int>(end) < check_len) {
    if (separator.empty()) {
      if (s[end] == ' ') {
        if (!in_white_space) {
          parts.emplace_back(make_pair(s.substr(start, end-start), start));
          in_white_space = true;
        }
      } else if (in_white_space) {
        start = end;
  ...
  if (start < s.length()) {
    parts.emplace_back(make_pair(s.substr(start), start));
  } else {
    parts.emplace_back(make_pair("", s.length()));
  }
  return parts;
}
...
string capitalize(const string& s, size_t start_word, size_t num_words) {
  ...
  if (!s.empty()) {
    ...
    auto index_s = string::npos;
    auto index_p = string::npos;
...
string strand(size_t length) {
  ...
  for (size_t n=0; n < length; ++n) {
    auto mnum = (rand() % 100);
    while (1) {
      while (mnum > 0 && (mnum < 13 || (mnum > 22 && mnum < 30) || (mnum > 55 &&
          mnum < 62) || mnum > 87)) {
        mnum /= 10;
      }
      if (mnum != 0) {
...
string to_title(const string& s) {
  static unordered_set<string> uncapitalized_words;
  if (uncapitalized_words.empty()) {
    ...
    for (size_t n = 0; n < words.size(); ++n) {
      if (words[n].first[0] >= 'A' && words[n].first[0] <= 'Z') {
        ...
        if (num_lower == 0) {
          ...
          if (n > 0 && n < last && uncapitalized_words.find(to_lower(words[n].
              first)) != uncapitalized_words.end()) {
        ...
        if (num_upper == 0 && (uncapitalized_words.find(words[n].first) ==
            uncapitalized_words.end() || n == 0 || n == last)) {
          title_s[words[n].second] -= 32;
        }
      }
    }
