...
int InputWMSSCObservationStream::read(unsigned char *buffer, size_t
    buffer_length) {
...
void WMSSCObservation::fill(const unsigned char *stream_buffer, bool
    fill_header_only) {
    ...
    if (wmssc.addl_data_indicator == 1) {
      if (yr >= 1961) {
        for (size_t n = 0; n < 12; ++n) {
          ...
          bits::get(stream_buffer, monthly_means[n].wmssc_addl_data.mois_flag,
              off+11, 1);
          ...
          bits::get(stream_buffer, monthly_means[n].wmssc_addl_data.n_pcp_1mm,
              off+34, 6);
          if (monthly_means[n].wmssc_addl_data.n_pcp_1mm == 63) {
            monthly_means[n].wmssc_addl_data.n_pcp_1mm = -9;
          }
          bits::get(stream_buffer, monthly_means[n].wmssc_addl_data.pcp_dep,
              off+40, 12);
        ...
        bits::get(stream_buffer, annual_wmssc_addl_data.pct_avg_sun, off+71,
            10);
...
SurfaceObservation::SurfaceAdditionalData WMSSCObservation::
    monthly_additional_data(size_t month_number) const {
...
SurfaceObservation::SurfaceReport WMSSCObservation::monthly_report(size_t
    month_number) const {
  if (month_number < 1 || month_number > 12) {
...
WMSSCObservation::WMSSCAdditionalData WMSSCObservation::
    monthly_WMSSC_additional_data(size_t month_number) const {
  if (month_number < 1 || month_number > 12) {
...
void WMSSCObservation::print(std::ostream& outs) const {
  ...
  for (size_t n = 0; n < 12; ++n) {
    ...
    switch (static_cast<int>(monthly_means[n].slp_indicator2)) {
      ...
      case 1: {
        outs << "  P: " << std::setw(6) << monthly_means[n].report.data.slp <<
            " at Z: " << std::setw(5) << monthly_means[n].z;
        break;
      }
      case 2: {
        outs << "  Z: " << std::setw(5) << monthly_means[n].z << " at P: " <<
            std::setw(6) << monthly_means[n].report.data.slp;
        break;
      }
    }
    outs << "  I: " << static_cast<int>(monthly_means[n].report.flags.slp) <<
        "/" << static_cast<int>(monthly_means[n].slp_indicator2) << "  STNP: "
        << std::setw(6) << monthly_means[n].report.data.stnp << "  T: " <<
        std::setw(5) << monthly_means[n].report.data.tdry << "  PCP: " <<
        std::setw(6) << monthly_means[n].addl_data.data.pcp_amt << endl;
  }
  outs << "   ANNUAL:  ";
  switch (static_cast<int>(annual_slp_indicator2)) {
    ...
    case 1: {
      outs << "  P: " << std::setw(6) << report_.data.slp << " at Z: " <<
          std::setw(5) << annual_z;
      break;
    }
    case 2: {
      outs << "  Z: " << std::setw(5) << annual_z << " at P: " << std::setw(6)
          << report_.data.slp;
      break;
    }
  }
  outs << "  I: " << static_cast<int>(report_.flags.slp) << "/" <<
      static_cast<int>(annual_slp_indicator2) << "  STNP: " << std::setw(6) <<
      report_.data.stnp << "  T: " << std::setw(5) << report_.data.tdry <<
      "  PCP: " << std::setw(6) << addl_data.data.pcp_amt << endl;
  if (static_cast<int>(wmssc.addl_data_indicator) == 1) {
    outs << " ADDITIONAL DATA:" << endl;
    if (date_time_.year() >= 1961) {
      for (size_t n = 0; n < 12; ++n) {
        outs << "   MONTH: " << std::setw(2) << (n+1) << "  T_DEP: " <<
            std::setw(5) << monthly_means[n].wmssc_addl_data.t_dep << "  MOIS: "
            << std::setw(5) << monthly_means[n].wmssc_addl_data.mois << " I: "
            << static_cast<int>(monthly_means[n].wmssc_addl_data.mois_flag) <<
            "  MOIS_DEP: " << std::setw(5) << monthly_means[n].wmssc_addl_data.
            mois_dep << "  #PCP>1MM: " << std::setw(2) << monthly_means[n].
            wmssc_addl_data.n_pcp_1mm << "  PCP_DEP: " << std::setw(4) <<
            monthly_means[n].wmssc_addl_data.pcp_dep << "  QUINT: " <<
            monthly_means[n].wmssc_addl_data.quint_pcp << "  #OBS: " <<
            std::setw(2) << monthly_means[n].wmssc_addl_data.nobs << "  SUN: "
            << std::setw(4) << monthly_means[n].wmssc_addl_data.hrs_sun <<
            "  %AVG: " << std::setw(3) << monthly_means[n].wmssc_addl_data.
            pct_avg_sun;
        if (monthly_means[n].wmssc_addl_data.sst > -99. || monthly_means[n].
            wmssc_addl_data.sst_dep > -99.)
          outs << "  SST: " << std::setw(5) << monthly_means[n].wmssc_addl_data.
              sst << "  SST_DEP: " << std::setw(5) << monthly_means[n].
              wmssc_addl_data.sst_dep;
        outs << endl;
      }
      outs << "   ANNUAL:  " << endl;
      outs << "  T_DEP: " << std::setw(5) << annual_wmssc_addl_data.t_dep <<
          "  MOIS: " << std::setw(5) << annual_wmssc_addl_data.mois << " I: " <<
          static_cast<int>(annual_wmssc_addl_data.mois_flag) << "  MOIS_DEP: "
          << std::setw(5) << annual_wmssc_addl_data.mois_dep << "  #PCP>1MM: "
          << std::setw(2) << annual_wmssc_addl_data.n_pcp_1mm << "  PCP_DEP: "
          << std::setw(4) << annual_wmssc_addl_data.pcp_dep << "  QUINT: " <<
          annual_wmssc_addl_data.quint_pcp << "  #OBS: " << std::setw(2) <<
          annual_wmssc_addl_data.nobs << "  SUN: " << std::setw(4) <<
          annual_wmssc_addl_data.hrs_sun << "  %AVG: " << std::setw(3) <<
          annual_wmssc_addl_data.pct_avg_sun;
      if (annual_wmssc_addl_data.sst > -99. || annual_wmssc_addl_data.sst_dep >
          -99.)
        outs << "  SST: " << std::setw(5) << annual_wmssc_addl_data.sst <<
            "  SST_DEP: " << std::setw(5) << annual_wmssc_addl_data.sst_dep;
      outs << endl;
    }
  }
}
...
void WMSSCObservation::print_header(std::ostream& outs, bool verbose) const {
  ...
  outs.precision(1);
  outs << "WMO#: " << location_.ID << "  SHIP: " << static_cast<int>(wmssc.
      ship_indicator) << "  SRC CODES: " << static_cast<int>(wmssc.data_source)
      << "/" << static_cast<int>(wmssc.id_source) << "  NAME: " << wmssc.name <<
      "  DATE: " << date_time_.to_string("%Y-%m") << "  LAT: " << std::setw(5)
      << location_.latitude << "  LON: " << std::setw(6) << location_.longitude
      << "  ELEV: " << std::setw(5) << location_.elevation << "  ADI: " <<
      static_cast<int>(wmssc.addl_data_indicator) << endl;
}
