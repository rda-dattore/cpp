...
bool export_to_json_ld(std::ostream& ofs,string dsnum,XMLDocument& xdoc,size_t indent_length)
{
  ...
  if (elist.size() > 0) {
    ...
    for (const auto& author : elist) {
      ...
      if (author_type == "authorPerson" || author_type.empty()) {
        ...
        ofs << "      " << local_indent << "\"givenName\": \"" << author.attribute_value("fname") << "\"," << endl;
        ofs << "      " << local_indent << "\"familyName\": \"" << author.attribute_value("lname") << "\"" << endl;
      }
      else {
        ofs << "      " << local_indent << "\"@type\": \"Organization\"," << endl;
        ofs << "      " << local_indent << "\"name\": \"" << author.attribute_value("name") << "\"" << endl;
      }
      ofs << "    " << local_indent << "}";
      if (elist.size() > 1) {
    ...
    while (query.fetch_row(row)) {
      ...
      if (name_parts.back() == "UNAFFILIATED INDIVIDUAL") {
        ...
        if (contact_parts.size() > 0) {
          ofs << "      " << local_indent << "\"@type\": \"Person\"," << endl;
          ofs << "      " << local_indent << "\"name\": \"" << contact_parts.front() << "\"" << endl;
          ofs << "    " << local_indent << "}";
          if (query.num_rows() > 1) {
        ...
        ofs << "      " << local_indent << "\"@type\": \"Organization\"," << endl;
        ofs << "      " << local_indent << "\"name\": \"" << substitute(name_parts.back(),", ","/") << "\"" << endl;
        ofs << "    " << local_indent << "}";
        if (query.num_rows() > 1) {
          if (num_rows < query.num_rows()) {
            ofs << ",";
  ...
  if (query.submit(server) == 0 && query.fetch_row(row)) {
    ...
    ofs << "    \"temporalCoverage\": \"" << sdate << "/" << edate << "\"," << endl;
  }
  ...
  if (is_grid) {
    ...
    if (min_west_lon < 9999.) {
      if (myequalf(min_west_lon,max_east_lon) && myequalf(min_south_lat,max_north_lat)) {
        ...
        ofs << "        \"box\": \"" << min_south_lat << "," << min_west_lon << " " << max_north_lat << "," << max_east_lon << "\"" << endl;
      }
    }
    ofs << "      }" << endl;
    ofs << "    }," << endl;
...
*/
  ...
  ofs << "    }," << endl;
  ofs << "    \"datePublished\": \"" << xdoc.element("dsOverview/publicationDate").content() << "\"" << endl;
  ofs << "  }" << endl;
  ofs << "</script>" << endl;
  return true;
}
