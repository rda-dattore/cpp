...
bool filled_inventory(std::string dsnum,std::string data_file,std::string type,std::stringstream& inventory)
{
  ...
  MySQL::LocalQuery query("select w.code,w.format_code,f.format,w.uflag from WGrML.ds"+dsnum2+"_webfiles2 as w left join WGrML.formats as f on f.code = w.format_code where id = '"+data_file+"'");
  MySQL::Row row;
  ...
  if (stat(inv_file.c_str(),&buf) != 0 || !versions_match) {
    query.set("distinct process","WGrML.ds"+dsnum2+"_processes","file_code = "+webid_code);
    ...
    query.set("select time_range_codes,grid_definition_codes,parameter,level_type_codes from WGrML.ds"+dsnum2+"_agrids2 where file_code = "+webid_code);
    if (query.submit(server) < 0) {
	myerror="database error - "+query.error();
	return false;
    }
    else if (query.num_rows() == 0) {
std::cerr << "no result returned - " << query.show() << std::endl;
exit(1);
    }
	...
	if (t.find(res[0]) == t.end()) {
	  ...
	  for (const auto& code : values) {
		...
		MySQL::LocalQuery tr_query("time_range","WGrML.time_ranges","code = "+strutils::itos(code));
		MySQL::Row row;
	...
	if (g.find(res[1]) == g.end()) {
	  ...
	  for (const auto& code : values) {
		...
		MySQL::LocalQuery gd_query("definition,defParams","WGrML.grid_definitions","code = "+strutils::itos(code));
		MySQL::Row row;
