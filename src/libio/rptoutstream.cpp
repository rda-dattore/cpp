...
int irstream::ignore() {
  if (file_buf_pos == file_buf_len) {
    if (icosstream != nullptr) {
      ...
      if (!fs.is_open()) {
        cerr << "Error: no irstream has been opened" << endl;
        exit(1);
      }
    ...
    if (checksum(file_buf.get(), block_len, (60+_flag*4), sum) != 0) {
      cerr << "Warning: checksum error on block number " << num_blocks << endl;
    }
...
{
  if (is_open()) {
    cerr << "Error: currently connected to another file stream" << endl;
    exit(1);
  }
...
int orstream::write(const unsigned char *buffer, size_t num_bytes) {
  if (!fs.is_open()) {
    cerr << "Error: no orstream has been opened" << endl;
    exit(1);
  }
...
int ocrstream::write(const unsigned char *buffer,size_t num_bytes)
{
  if (ocosstream == nullptr) {
    cerr << "Error: no ocrstream has been opened" << endl;
    exit(1);
  }
  if (num_bytes > file_buf_len) {
...
{
  if (ocrstream::flush() == error) {
    cerr << "Warning: error while flushing output rptoutstream" << endl;
  }
...
int iocrstream::ignore()
{
  switch (stream_type) {
    ...
    case 1:
    {
      cerr << "Warning: ignore should not be called on a stream opened for writing" << endl;
    }
  }
...
int iocrstream::peek()
{
  switch (stream_type) {
    ...
    case 1:
    {
      cerr << "Warning: peek should not be called on a stream opened for writing" << endl;
      exit(1);
    }
  }
  return bfstream::error;
...
int iocrstream::read(unsigned char *buffer,size_t buffer_length)
{
  switch (stream_type) {
    ...
    case 1:
    {
      cerr << "Warning: read should not be called on a stream opened for writing" << endl;
      exit(1);
    }
  }
  return bfstream::error;
...
int iocrstream::write(const unsigned char *buffer,size_t num_bytes)
{
  switch (stream_type) {
    case 0:
    {
      cerr << "Warning: write should not be called on a stream opened for reading" << endl;
      exit(1);
    }
    case 1:
    {
...
void iocrstream::write_eof()
{
  switch (stream_type) {
    case 0:
    {
      cerr << "Warning: write_eof should not be called on a stream opened for reading" << endl;
      exit(1);
    }
    case 1:
    {
