...
void add_parameter_to_unique_tables(GRIBGrid *grid,short edition,GridData& grid_data,HouseKeeping& hk,xmlutils::LevelMapper& level_mapper,my::map<TimeEntry>& unique_time_table,off_t record_offset,off_t record_length)
{
  NetCDF::UniqueVariableEntry ve;
  NetCDF::UniqueVariableDataEntry data;
  NetCDF::UniqueVariableTimeEntry times;
  NetCDF::UniqueVariableStringEntry se;
  TimeEntry te;
  std::string trange;
  ...
  if (edition == 2) {
    ...
    te.key = grid->forecast_date_time().to_string("%Y-%m-%d %H:%MM:%SS");
    if (!unique_time_table.found(te.key, te)) {
      unique_time_table.insert(te);
    }
    std::tie(p, f, l) = grib_utils::grib2_product(reinterpret_cast<GRIB2Grid *>(
        grid));
    auto idx = p.find(" (initial+");
  ...
  if (!hk.unique_variable_table.found(ve.key,ve)) {
    ve.data.reset(new NetCDF::UniqueVariableEntry::Data);
    ...
    if (edition == 2) {
      ve.data->product_description = p;
    }
    ve.data->cell_methods=grid_data.cell_methods;
    ve.data->is_layer=is_layer;
    hk.unique_variable_table.insert(ve);
  } else if (!fixed_product && edition == 2 && p !=
      ve.data->product_description) {
    auto sp1=strutils::split(p);
    auto sp2=strutils::split(ve.data->product_description);
  ...
  data.time_entry.reset(new NetCDF::UniqueVariableTimeEntry);
  data.time_entry->first_valid_datetime.reset(new DateTime);
  *(data.time_entry->first_valid_datetime) = f;
  data.time_entry->last_valid_datetime.reset(new DateTime);
  *(data.time_entry->last_valid_datetime) = l;
  ...
  *data.record_length=record_length;
  data.level.reset(new NetCDF::UniqueVariableLevelEntry);
...
void scan_latitudes(const Grid *source_grid,float **latrange,float **latnonrec,Grid::LL_SubsetDefinition& subset_definition)
{
  std::ifstream ifs;
  char line[32768];
  int n;
  ...
  if ( (subset_definition.latitude.north-subset_definition.latitude.south) > 200.) {
  ...
  subset_definition.y.min=0x7fffffff;
  subset_definition.y.max=0;
  subset_definition.y.num=0;
  if (source_grid->definition().type == Grid::Type::latitudeLongitude) {
    *latnonrec=new float[source_grid->dimensions().y];
    for (n=0,subset_definition.y.num=0; n < source_grid->dimensions().y; n++) {
      if ((reinterpret_cast<GRIBGrid *>(const_cast<Grid *>(source_grid)))->scan_mode() == 0x0) {
        (*latnonrec)[subset_definition.y.num]=source_grid->definition().slatitude-n*source_grid->definition().laincrement;
      }
      else {
        (*latnonrec)[subset_definition.y.num]=source_grid->definition().slatitude+n*source_grid->definition().laincrement;
      }
      if (subset_definition.latitude.south == subset_definition.latitude.north) {
        dist=fabsf((*latnonrec)[subset_definition.y.num]-subset_definition.latitude.south);
        if (dist < min_dist) {
          min_dist=dist;
          subset_definition.y.min=subset_definition.y.max=n;
          (*latrange)[0]=(*latrange)[1]=(*latnonrec)[subset_definition.y.num];
        }
      }
      else {
        if (((*latnonrec)[subset_definition.y.num] > subset_definition.latitude.south || floatutils::myequalf((*latnonrec)[subset_definition.y.num],subset_definition.latitude.south,0.0001)) && ((*latnonrec)[subset_definition.y.num] < subset_definition.latitude.north || floatutils::myequalf((*latnonrec)[subset_definition.y.num],subset_definition.latitude.north,0.0001))) {
          if (n < subset_definition.y.min) {
            subset_definition.y.min=n;
            if ( (subset_definition.latitude.north-subset_definition.latitude.south) < 200.) {
              if ((reinterpret_cast<GRIBGrid *>(const_cast<Grid *>(source_grid)))->scan_mode() == 0x0) {
                (*latrange)[1]=(*latnonrec)[subset_definition.y.num];
              }
              else {
                (*latrange)[0]=(*latnonrec)[subset_definition.y.num];
              }
            }
          }
          if (n > subset_definition.y.max) {
            subset_definition.y.max=n;
            if ( (subset_definition.latitude.north-subset_definition.latitude.south) < 200.) {
              if ((reinterpret_cast<GRIBGrid *>(const_cast<Grid *>(source_grid)))->scan_mode() == 0x0) {
                (*latrange)[0]=(*latnonrec)[subset_definition.y.num];
              }
          ...
          subset_definition.y.num++;
        }
      }
    }
  }
  else if (source_grid->definition().type == Grid::Type::gaussianLatitudeLongitude) {
    ...
    *latnonrec=new float[source_grid->definition().num_circles*2];
    ...
    subset_definition.y.num=0;
    while (!ifs.eof()) {
      (*latnonrec)[subset_definition.y.num]=atof(line);
      if (subset_definition.latitude.south == subset_definition.latitude.north) {
        dist=fabsf((*latnonrec)[subset_definition.y.num]-subset_definition.latitude.south);
        if (dist < min_dist) {
          min_dist=dist;
        ...
        if ((*latnonrec)[subset_definition.y.num] >= subset_definition.latitude.south && (*latnonrec)[subset_definition.y.num] <= subset_definition.latitude.north) {
          if (n < subset_definition.y.min) {
            subset_definition.y.min=n;
            if ( (subset_definition.latitude.north-subset_definition.latitude.south) < 200.) {
              (*latrange)[1]=(*latnonrec)[subset_definition.y.num];
...
void scan_longitudes(const Grid *source_grid,float **lonrange,float **lonnonrec,Grid::LL_SubsetDefinition& subset_definition)
{
  Grid::GridDefinition def;
  int n;
  ...
  if ( (subset_definition.longitude.east-subset_definition.longitude.west) > 400.) {
...
// data are organized by east longitude
      if (def.slongitude >= 0. && def.elongitude >= 0.) {
        ...
        if (subset_definition.longitude.east >= 0. && subset_definition.longitude.west < 0.) {
          if (subset_definition.longitude.east > 0. || (floatutils::myequalf(subset_definition.longitude.east,0.,0.01) && subset_definition.longitude.east >= def.slongitude)) {
// subset crosses greenwich meridian
            subset_definition.crosses_greenwich=true;
            if (subset_definition.longitude.east < 0.) {
              subset_definition.longitude.east+=360.;
            }
          }
          else {
            if (subset_definition.longitude.east <= 0.) {
        ...
        if (subset_definition.longitude.west < 0.) {
          subset_definition.longitude.west+=360.;
        }
      }
// data are organized by west longitude
      else {
        if (subset_definition.longitude.east < subset_definition.longitude.west) {
...
exit(1);
  ...
  *lonnonrec=new float[source_grid->dimensions().x];
  subset_definition.x.min=0x7fffffff;
  subset_definition.x.max=0;
  if (subset_definition.crosses_greenwich) {
    for (n=0,subset_definition.x.num=0; n < source_grid->dimensions().x; ++n) {
      (*lonnonrec)[subset_definition.x.num]=def.slongitude+n*def.loincrement;
      if ((*lonnonrec)[subset_definition.x.num] > subset_definition.longitude.west || floatutils::myequalf((*lonnonrec)[subset_definition.x.num],subset_definition.longitude.west,0.01)) {
        if ((source_lons_are_east && (*lonnonrec)[subset_definition.x.num] > 180.) || floatutils::myequalf((*lonnonrec)[subset_definition.x.num],180.,0.01)) {
          (*lonnonrec)[subset_definition.x.num]-=360.;
        }
        if (n < subset_definition.x.min) {
          subset_definition.x.min=n;
          if ( (subset_definition.longitude.east-subset_definition.longitude.west) < 400.) {
            (*lonrange)[0]=(*lonnonrec)[subset_definition.x.num];
          }
        }
        subset_definition.x.num++;
      }
    }
    for (n=0; n < source_grid->dimensions().x; n++) {
      (*lonnonrec)[subset_definition.x.num]=def.slongitude+n*def.loincrement;
      if ((*lonnonrec)[subset_definition.x.num] < subset_definition.longitude.east || floatutils::myequalf((*lonnonrec)[subset_definition.x.num],subset_definition.longitude.east,0.01)) {
        if (n > subset_definition.x.max) {
          subset_definition.x.max=n;
          if ( (subset_definition.longitude.east-subset_definition.longitude.west) < 400.) {
            (*lonrange)[1]=(*lonnonrec)[subset_definition.x.num];
          }
        }
        subset_definition.x.num++;
      }
    ...
    for (n=0,subset_definition.x.num=0; n < source_grid->dimensions().x; ++n) {
      (*lonnonrec)[subset_definition.x.num]=def.slongitude+n*def.loincrement;
      if (subset_definition.longitude.west == subset_definition.longitude.east) {
        dist=fabsf((*lonnonrec)[subset_definition.x.num]-subset_definition.longitude.west);
        if (dist < min_dist) {
          min_dist=dist;
          subset_definition.x.min=subset_definition.x.max=n;
          (*lonrange)[0]=(*lonrange)[1]=(*lonnonrec)[subset_definition.x.num];
        }
      }
      else {
        if (((*lonnonrec)[subset_definition.x.num] > subset_definition.longitude.west || floatutils::myequalf((*lonnonrec)[subset_definition.x.num],subset_definition.longitude.west,0.0001)) && ((*lonnonrec)[subset_definition.x.num] < subset_definition.longitude.east || floatutils::myequalf((*lonnonrec)[subset_definition.x.num],subset_definition.longitude.east,0.0001))) {
          if (n < subset_definition.x.min) {
            subset_definition.x.min=n;
            if ( (subset_definition.longitude.east-subset_definition.longitude.west) < 400.) {
              (*lonrange)[0]=(*lonnonrec)[subset_definition.x.num];
  ...
  else if (subset_definition.longitude.west == subset_definition.longitude.east) {
    (*lonnonrec)[0]=(*lonrange)[0];
    subset_definition.x.num=1;
  }
}

bool compare_times(NetCDF::UniqueVariableTimeEntry& left,NetCDF::UniqueVariableTimeEntry& right)
{
...
bool compare_variable_data(const NetCDF::UniqueVariableDataEntry& left,const NetCDF::UniqueVariableDataEntry& right)
{
  int status;
  NetCDF::UniqueVariableLevelEntry llev,rlev;
...
std::string level_list_key(NetCDF::UniqueVariableEntry& ve)
{
...
void write_netcdf_header_from_grib_file(InputGRIBStream& istream,OutputNetCDFStream& ostream,GridData& grid_data,HouseKeeping& hk,std::string path_to_level_map)
{
  std::ifstream ifs;
  ...
  size_t t_dimids[3];
  grib_utils::StringEntry se;
  int nsteps=-1;
  std::list<std::list<NetCDF::UniqueVariableTimeEntry> *> time_dims;
    ...
    for (n=0; n < msg->number_of_grids(); n++) {
      ...
      if (include_parameter) {
        if (istream.number_read() == 1 && empty_ref_def) {
          ...
          if (grid->definition().type == Grid::Type::latitudeLongitude || grid->definition().type == Grid::Type::gaussianLatitudeLongitude) {
            ref_def=grid->definition();
            ref_dim=grid->dimensions();
          }
          else {
            std::cerr << "Error: no conversion for grid type " << static_cast<int>(grid->definition().type) << " yet" << std::endl;
            exit(1);
            }
          empty_ref_def=false;
        }
...
std::cerr << n << std::endl;
std::cerr << static_cast<int>(grid->definition().type) << " " << static_cast<int>(ref_def.type) << " " << (grid->definition().type == ref_def.type) << std::endl;
...
// add any level dimensions
  ...
  ostream.add_dimension("lat",grid_data.subset_definition.y.num);
...
// write the non-record variables
  if (grid_data.record_flag == 0) {
    ...
    std::list<NetCDF::UniqueVariableTimeEntry> *x;
...
int convert_grid_to_netcdf(Grid *source_grid, Grid::Format format,OutputNetCDFStream *ostream,GridData& grid_data,std::string path_to_level_map)
{
  std::ifstream ifs;
  std::vector<NetCDF::Variable> vars;
  ...
  NetCDF::VariableData gridpoints,nctime,char_data;
  DateTime dateTime,firstValidDateTime,lastValidDateTime;
  int range_values[2];
  float *latrange=nullptr,*lonrange=nullptr;
  size_t dim_ids[3];
  double **lat,**lon;
  std::string trange,var_name,var_description,var_units,lev_description,sdum;
  size_t t_dimids[3];
  xmlutils::LevelMapper level_mapper(path_to_level_map);
  bool is_layer;
...
// global attributes
        ...
        ostream->add_global_attribute("Creator","NCAR - CISL RDA (dattore)");
// cell methods?
        if (format == Grid::Format::grib2) {
          grid_data.cell_methods=GRIB2_cell_methods(*g2);
        }
// add global attributes
// add dimensions
        ostream->add_dimension("time",0);
        if (source_grid->definition().type == Grid::Type::latitudeLongitude || source_grid->definition().type == Grid::Type::gaussianLatitudeLongitude) {
          ...
          std::cerr << "Error: no conversion for grid type " << static_cast<int>(source_grid->definition().type) << " yet" << std::endl;
          exit(1);
        }
        ostream->add_dimension("tstrlen",10);
        if (is_bounded_time(grid_data.cell_methods)) {
...
// data variable
        ...
        var_name = build_grib_parameter_ID(g, *grid_data.parameter_mapper,
            is_layer, trange);
        if (format == Grid::Format::grib) {
          var_description = g->parameter_cf_keyword(
              *grid_data.parameter_mapper);
          if (var_description.empty()) {
            var_description = g->parameter_description(
                *grid_data.parameter_mapper);
          } else {
          ...
          var_description = g2->parameter_cf_keyword(
              *grid_data.parameter_mapper);
          if (var_description.empty()) {
            var_description = g2->parameter_description(
                *grid_data.parameter_mapper);
          } else {
            var_description = "CF:" + var_description;
          }
          std::string p;
          DateTime f, l;
          std::tie(p, f, l) = grib_utils::grib2_product(g2);
          if (!p.empty()) {
            auto idx = p.find(" (initial+");
            if (idx != std::string::npos) {
              p = p.substr(0, idx);
            }
            ostream->add_variable_attribute(var_name, "product_description", p);
          }
        ...
        if (strutils::has_beginning(var_description, "CF:")) {
          ...
          ostream->add_variable_attribute(var_name, "long_name",
              var_description);
        }
        ostream->add_variable_attribute(var_name, "units",
            convert_units_to_CF(var_units));
        ostream->add_variable_attribute(var_name, "level", lev_description);
        ostream->add_variable_attribute(var_name, "_FillValue",
            static_cast<float>(Grid::MISSING_VALUE));
        if (!grid_data.cell_methods.empty()) {
          ostream->add_variable_attribute(var_name, "cell_methods",
              grid_data.cell_methods);
        }
      ...
      if (g->scan_mode() == 0x0) {
        for (n=grid_data.subset_definition.y.min,l=0; n <= grid_data.subset_definition.y.max; ++n) {
          if (grid_data.subset_definition.crosses_greenwich) {
            for (m=grid_data.subset_definition.x.min; m < source_grid->dimensions().x; ++m) {
              gridpoints.set(l++,source_grid->gridpoint(m,n));
            }
...
short extract_record(FILE *fp,NetCDF::UniqueVariableDataEntry data,unsigned char **buffer,int& buf_len)
{
  short edition;
...
void convert_grib_file_to_netcdf_as_non_record_variables(FILE *fp,OutputNetCDFStream& ostream,Grid::LL_SubsetDefinition& subset_definition,xmlutils::ParameterMapper& parameter_mapper,my::map<NetCDF::UniqueVariableEntry>& unique_variable_table)
{
...
// check for greenwich meridian existing twice (i.e. 0 -> 360)
          ...
          npoints=0;
          for (n=subset_definition.y.min; n <= subset_definition.y.max; n++) {
            if (subset_definition.crosses_greenwich) {
              max_x= (greenwich_appears_twice) ? grid->dimensions().x-1 : grid->dimensions().x;
              for (m=subset_definition.x.min; m < max_x; ++m) {
                gridpoints[npoints++]=grid->gridpoint(m,n);
              }
              for (m=0; m <= subset_definition.x.max; ++m) {
                gridpoints[npoints++]=grid->gridpoint(m,n);
              }
            }
            else {
              for (m=subset_definition.x.min; m <= subset_definition.x.max; ++m) {
                gridpoints[npoints++]=grid->gridpoint(m,n);
              }
            }
          }
...
void convert_grib_file_to_netcdf_as_record_variables(FILE *fp,OutputNetCDFStream& ostream,DateTime& ref_date_time,std::string& cell_methods,Grid::LL_SubsetDefinition& subset_definition,xmlutils::ParameterMapper& parameter_mapper,std::string path_to_level_map,my::map<NetCDF::UniqueVariableEntry>& unique_variable_table)
{
  unsigned char *buffer=nullptr;
  int buf_len=0;
  GRIBMessage *msg=nullptr;
  ...
  short edition;
  NetCDF::UniqueVariableEntry ve;
  std::deque<std::list<NetCDF::UniqueVariableDataEntry>::iterator> it,end;
  GridData grid_data;
  Grid::Format format;
  int n,m,l,x;
  std::string trange,paramID;
    ...
    for (auto& key : unique_variable_table.keys()) {
      ...
      for (n=0; n < static_cast<int>(ve.data->unique_levels.size()); ++n) {
        ...
        if (edition == 1) {
          ...
          format=Grid::Format::grib;
        }
        else if (edition == 2) {
          if (msg == nullptr) {
            msg=new GRIB2Message;
          }
          format=Grid::Format::grib2;
        }
        else {
          std::cerr << "Error: GRIB edition '" << edition << "' is not supported" << std::endl;
          exit(1);
