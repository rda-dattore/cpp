...
const std::string DATACITE_HOSTING_INSTITUTION="University Corporation For Atmospheric Research (UCAR):National Center for Atmospheric Research (NCAR):Computational and Information Systems Laboratory (CISL):Information Services Division (ISD):Data Engineering and Curation Section (DECS)";
...
extern std::string primary_size(std::string dsnum,MySQL::Server& server);

extern void add_replace_from_element_content(XMLDocument& xdoc,std::string xpath,std::string replace_token,TokenDocument& token_doc,std::string if_key = "");
extern void add_to_resolution_table(double lon_res,double lat_res,std::string units,my::map<Entry>& resolution_table);
extern void fill_geographic_extent_data(MySQL::Server& server,std::string dsnum,XMLDocument& dataset_overview,double& min_west_lon,double& min_south_lat,double& max_east_lon,double& max_north_lat,bool& is_grid,std::vector<HorizontalResolutionEntry> *hres_list = nullptr,my::map<Entry> *unique_places_table = nullptr);
...
extern bool export_to_thredds(std::ostream& ofs,std::string ident,XMLDocument& xdoc,size_t indent_length);
