...
{
public:
  enum {tsr_format=1,cards_format,ussr_format,adp_format,french_format,datsav_format,navyspot_format,kunia_format,ruship_format,galapagos_format,argentina_format,mit_format,china_format,galapagoshr_format};
  struct RaobLevel {
    RaobLevel() : pressure(0.),temperature(0.),moisture(0.),wind_speed(0.),height(0),wind_direction(0),type(0),ucomp(0.),vcomp(0.),time(0.),level_quality(' '),time_quality(' '),pressure_quality(' '),height_quality(' '),temperature_quality(' '),moisture_quality(' '),wind_quality(' ') {}

    float pressure,temperature,moisture,wind_speed;
    int height,wind_direction;
    size_t type;
    float ucomp,vcomp,time;
    char level_quality,time_quality,pressure_quality,height_quality,temperature_quality,moisture_quality,wind_quality;
  };
  Raob() : nlev(0),levels() {}
...
//  InputTsraobStream(const InputTsraobStream& source) { *this=source; }
  int ignore();
  int peek();
...
//  OutputTsraobStream(const OutputTsraobStream& source) { *this=source; }
...
public:
  struct TsrFlags {
    TsrFlags() : ubits(0),format(0),source(0),height_temperature_status(0),wind_status(0),surface_level(0),wind_units(0),moisture_units(0),additional_data(false) {}

    short ubits,format,source,height_temperature_status,wind_status,surface_level,wind_units,moisture_units;
  ...
  void set_level(short level_number,RaobLevel level,size_t recompute_bits) { levels[level_number]=level; rcb[level_number]=recompute_bits; }
  void set_location(ObservationLocation location) { location_=location; }
  void set_quality_indicators(float release_time[2],short quality_indicators[9]);
...
private:
  ...
  struct Tsr {
    Tsr() : flags(),deck(0),ind(),nind(0),trel(),qual_is_ascii(),format_flag(1) {}
...
//extern char getQuality(char qflag);

class InputCardsRaobStream : public idstream
{
public:
  InputCardsRaobStream() : first_card(),at_eof(false) {}
  InputCardsRaobStream(const char *filename) : InputCardsRaobStream() { open(filename); }
  void close();
  int ignore();
...
//  InputUSSRRaobStream(const InputUSSRRaobStream& source) { *this=source; }
  int ignore();
  int peek();
  int read(unsigned char *buffer,size_t buffer_length);
};
...
{
public:
  InputFrenchRaobStream() : file_buf(nullptr),block_size(0),file_buf_pos(0),file_buf_len(0) {}
  InputFrenchRaobStream(const char *filename) : InputFrenchRaobStream() { open(filename); }
  void close();
  int ignore();
  bool open(const char *filename);
  int peek();
...
private:
  struct DRaob {
    DRaob() : type(),call(),wind_type(),num_addl(0),num_rmks(0),num_reject(0),thermo_code(0),wind_code(0),qual_rate(0),source(0) {}
...
extern int DATSAVRaobHeightCompare(Raob::RaobLevel& left,Raob::RaobLevel& right);
extern int DATSAVRaobPressureCompare(Raob::RaobLevel& left,Raob::RaobLevel& right);
...
public:
  ...
  InputKuniaRaobStream(const char *filename) : InputKuniaRaobStream() { open(filename); }
//  InputKuniaRaobStream(const InputKuniaRaobStream& source) : InputKuniaRaobStream() { *this=source; }
  int ignore();
  bool open(const char *filename);
  int peek();
  int read(unsigned char *buffer,size_t buffer_length);
...
public:
  ...
  size_t copy_to_buffer(unsigned char *output_buffer,size_t buffer_length) const;
  void fill(const unsigned char *stream_buffer,bool fill_header_only);
  short moisture_flag(size_t level_number) { return kunia.mois_flag[level_number]; }
  void print(std::ostream& outs) const;
  void print_header(std::ostream& outs,bool verbose) const;
...
//  InputRussianShipRaobStream(const InputRussianShipRaobStream& source) { *this=source; }
  int ignore();
  int peek();
  int read(unsigned char *buffer,size_t buffer_length);
...
{
public:
  RussianShipRaob() : octant(0),type(0),device(0),call_sign(0),wind_quality2() {}
  size_t copy_to_buffer(unsigned char *output_buffer,size_t buffer_length) const;
  void fill(const unsigned char *stream_buffer,bool fill_header_only);
  void print(std::ostream& outs) const;
  void print_header(std::ostream& outs,bool verbose) const;
...
public:
  ...
  InputGalapagosSoundingStream(const char *filename) : InputGalapagosSoundingStream() { open(filename); }
  int ignore();
  int peek();
  int read(unsigned char *buffer,size_t buffer_length);
...
extern int sortGalapagosByPressure(Raob::RaobLevel& left,Raob::RaobLevel& right);

class InputArgentinaRaobStream : public idstream
{
public:
  InputArgentinaRaobStream() : last_ID(),last_record() {}
  InputArgentinaRaobStream(const char *filename) : InputArgentinaRaobStream() { open(filename); }
//  InputArgentinaRaobStream(const InputArgentinaRaobStream& source) : InputArgentinaRaobStream() { *this=source; }
  int ignore();
  int peek();
  int read(unsigned char *buffer,size_t buffer_length);
...
extern int compareArgentinaPressures(ArgentinaRaob::SortEntry& left,ArgentinaRaob::SortEntry& right);

class InputBrazilSoundingStream: public idstream
{
public:
  InputBrazilSoundingStream() : sline(),date_time() {}
  InputBrazilSoundingStream(const char *filename) : InputBrazilSoundingStream() { open(filename); }
//  InputBrazilSoundingStream(const InputBrazilSoundingStream& source) : InputBrazilSoundingStream() { *this=source; }
  int ignore();
  int peek();
  int read(unsigned char *buffer,size_t buffer_length);
...
//  InputMITRaobStream(const InputMITRaobStream& source) { *this=source; }
  int ignore();
  int peek();
  int read(unsigned char *buffer,size_t buffer_length);
};
...
//  InputChinaRaobStream(const InputChinaRaobStream& source) { *this=source; }
  int ignore();
  int peek();
  int read(unsigned char *buffer,size_t buffer_length);
};
